...
terminalVelocity = 50
maxJump = 2.0
jump = math.sqrt(2 * gravity * maxJump)
...
class Model(object):
    ...
    def addCube(self, location, vector):
        pf.cube(location, GL_QUADS, self.batch, vector)
    #Adds rectangles of different shapes based. One goes forward and one to the side.
    def addRectanglex(self, location, vector):
        pf.rectanglex(location, GL_QUADS, self.batch, vector, rotation=1)
        
    def addRectanglez(self, location, vector):
        pf.rectanglez(location, GL_QUADS, self.batch, vector, rotation=1)
    #This will make the words "SEND NUDES" whereever you are standing. 
    #This is a very important feature, of course.
    def addNudes(self, location):
        pf.nudes(location, GL_QUADS, self.batch)
    #Adds a small "Hallway" made up of rectangles and cubes.
    #Mostly pointless, unless snapping were to be added for building game related reasons.
    def addHallway(self, location, vector):
        pf.hallway(location, GL_QUADS, self.batch, vector)
...
class unGUI(pyglet.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        #Sets flying, exclusive mouse, etc to defaults. Most are fairly self explanatory. 
        self.dy = 0
        self.gravity = 20
        self.flying = False
        ...
        #This displays the direction that you're facing on a label, or rather, this sets it to blank. 
        #The direction(Forward, Right, Down, Backward and Up, Middle and Down) are set elsewhere based on camera movement.
        #The camera direction system is very far from perfect. That needs improvement. 
        #See on_mouse_motion for the rest of it.
        self.angley = ''
        self.direction = ''
        #This sets the default selection. Selections are changed later in the on_key_press section.
        #Should have it set to cycle(perhaps with a display of non-selected types) but that's far from important.
        self.selected = 'Cubes'
        ...
        if (os.path.exists('config') is False or os.path.exists('config\config.txt') is False):
            #If the path /config/ in the main folder(should default to the script folder, or exe if one were to be made)
            #Or if the config.txt file doesn't exist.
            #userSettings sets the current(default) values to add to the config file. Then it sends it down to playerConfig
            #which is what actually makes the file.
            #Again, not sure this is the best way to do this.
            userSettings = {'width': self.width, 'height': self.height, 'fullscreen': self.fullscreen, 'vsync': self.vsync}
            playerConfig().configFile(userSettings)
        #This checks for saves. There is little to no reason for character saves yet, but here it is.
        if (os.path.exists('saves') is False or os.path.exists('saves\character.txt') is False):
            ...
            with open('saves\character.txt') as keybind:
                #The keybindings don't work. Not really sure how to use them with Pyglets key system.
                keybind = keybind.read()
                keybind = json.loads(keybind)
        #set the model/batch rendering class from earlier to Model()
        self.model = Model()
        
        #This is the label in the top left. FPS display, x, y, z, version info and controls, etc. Text is set down in the draw_label function.
    ...
    def get_motion_vector(self):
        #This is the main movement function. It checks the value of playerMove and applies movement. 
        #The direction is based on what the value is, the value ranges from -1 to 1.
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.exclusive:
            ...
            #If you click with the left mouse button, it checks to see what the selected value is.
            ...
            if button == mouse.LEFT:
                ...
                #This draws the words SEND NUDES. Obviously a very important feature.
    ...
    def on_mouse_motion(self, x, y, dx, dy):
        if self.exclusive:
            ...
            #It changes the self.direction value based on your direction, to be later displayed on a label.
            #The way it decides direction is very poor, and there are some "blind spots". Mostly works though.
            vector = self.get_sight_vector()
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        #WASD controls. Jumping. Other assorted controls.
            ...
            #Resets your position to 0, 0, 0. Or whatever the default is(which is 0, 0, 0).
            self.position = self.spawnLoc
            self.rotation = self.spawnRot
        elif symbol == key.Q:
            #This stops any falling. Not very useful. Was a planned feature early on when I was actually
            #focused on making a game.
            self.dy = 0
            self.gravity = 0
        #Toggles resolution/fullscreen. Fullscreen may default to the wrong monitor if you have multiple.
        elif symbol == key.F1:
            ...
            #This is just where I've thrown functions I wanted to test quickly in the past. Technically useless.
            print("T for Testing.")
        #This chooses the object to make when you click.
        #To do, set it as a toggle with a display of some kind. That might be a bit much effort unless you're making a building game.
        elif symbol == key._1:
            self.selected = 'Cubes'
        elif symbol == key._2:
    ...
    def _update(self, dt):
        ...
        speed = flyingSpeed if self.flying == True else walkingSpeed
        ...
        #Checks if you're flying, if not, makes you fall until hitting the floor.
        #The "floor" in this case is just a y value of 0. There isn't actually a floor, or in fact, any collision.
        if self.flying == False:
            if y >= 0:
                self.dy -= dt * self.gravity
                self.dy = max(self.dy, -terminalVelocity)
                dy += self.dy * dt
            if y < 0:
    ...
    def on_resize(self, width, height):
        #The resize function. Makes sure that things are moved to account for new window size.
    ...
    def set3d(self):
        ...
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        gluPerspective(90, width / float(height), 0.1, 60)
        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()
        x, y = self.rotation
        glRotatef(x, 0, 1, 0)
    ...
    def draw_label(self):
        ...
        self.label.text = '%02d (%.2f, %.2f, %.2f)(%.2f, %.2f, %.2f)\nF1 changes resolution.\nF2 toggles fullscreen\n\n%s\n%s : %s' \
            % (pyglet.clock.get_fps(), x, y, z, dx, dy, dz, self.selected, self.direction, self.angley)
        self.label.draw()
...
class playerConfig():
    def configFile(self, usersettings):
        #This contains various JSON things for reading, editing and creating the config file.
        ...
        if os.path.exists(filename) is False:
            ...
            with open(filename, 'r+') as userConfig:
                firstset = json.dumps({'display settings': 
    ...
    def charDef(self):
        ...
        if os.path.exists(filename) is False:
            ...
            with open(filename, 'r+') as charData:
                ...
                defaultChar = json.dumps({'Character':
...
class main():
    def __init__(self):
        #This will check to see if there's a config file, if there is, applies the settings.
        ...
        window = unGUI(width=width, height=height, caption='Untitled: ', resizable=True, fullscreen=fullscreen, vsync=vsync)
        setup()
        pyglet.app.run()
